...
class ModelArguments:
    ...
    ori_model_path: str = field(
        metadata={"help": "Path to original pretrained model or model identifier from huggingface.co/models"}
    )
    ...
    T: Optional[int] = field(
        default=None, metadata={"help": "temperature in distilling"}
    )
...
def main():
    ...
    # we load the old init model
    ori_dict = ori_model.state_dict()
    cur_dict = model.state_dict()
    import torch
    ori_tuned_model = torch.load(f"{model_args.teacher_path}/pytorch_model.bin")
    
    for k in ori_dict.keys():
        cur_dict[k] = cur_dict[k].cuda() + (ori_tuned_model[k].cuda() - ori_dict[k].cuda())
