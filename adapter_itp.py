...
class ModelArguments:
    ...
    ori_model_path1: str = field(
        metadata={"help": "lhs backbone model to be interpolated"}
    )
    ori_model_path2: str = field(
        metadata={"help": "rhs backbone model to be interpolated"}
    )
...
def main():
    ...
    files = [f for f in os.listdir(training_args.output_dir) if os.path.isfile(os.path.join(training_args.output_dir, f))]
    ...
    logging.basicConfig(
        ...
        datefmt="%m/%d/%Y %H:%M:%S",
        handlers=[logging.FileHandler(os.path.join(training_args.output_dir, "log.txt")),
    ...
    if delta_args.delta_type.lower() != "none":
        ...
        delta_model.freeze_module(set_state_dict = False)
    ...
    flat1 = torch.load(os.path.join(model_args.path1, "pytorch_model.bin"))
    config1 = AutoConfig.from_pretrained(
        model_args.ori_model_path1,
        num_labels=num_labels,
    ...
    if delta_args.delta_type.lower() != "none":
        from opendelta import AutoDeltaConfig
        from opendelta.auto_delta import AutoDeltaModel
        if "train_itp" not in flat1:
            delta_model1 = AutoDeltaModel.from_finetuned(model_args.path1, backbone_model=model1)
        else:
            delta_config1 = AutoDeltaConfig.from_dict(vars(delta_args))
            delta_model1 = AutoDeltaModel.from_config(delta_config1, backbone_model=model1)
        delta_model1.freeze_module(set_state_dict = False)
        delta_model1.log(delta_ratio=True, trainable_ratio=True, visualization=True)
    # make the flat pet into tensor again
    if "train_itp" in flat1:
        flat1 = flat1["train_itp"]
        tmp_dict1 = model1.state_dict()
        keys = [k if "adapter" in k or "classifier" in k else None for k in tmp_dict1.keys()]
        shapes = [tmp_dict1[k].size() for k in keys]
        lens = [tmp_dict1[k].numel() for k in keys]
        pos = 0
        for i in range(len(keys)):
            param = flat1[pos:pos+lens[i]].view(shapes[i])
            tmp_dict1[keys[i]] = param
            pos += lens[i]
        model1.load_state_dict(tmp_dict1)

    flat2 = torch.load(os.path.join(model_args.path2, "pytorch_model.bin"))
    config2 = AutoConfig.from_pretrained(
        model_args.ori_model_path2,
        num_labels=num_labels,
        cache_dir=model_args.cache_dir,
    )
    ...
    if delta_args.delta_type.lower() != "none":
        from opendelta import AutoDeltaConfig
        from opendelta.auto_delta import AutoDeltaModel
        if "train_itp" not in flat2:
            delta_model2 = AutoDeltaModel.from_finetuned(model_args.path2, backbone_model=model2)
        else:
            delta_config2 = AutoDeltaConfig.from_dict(vars(delta_args))
            delta_model2 = AutoDeltaModel.from_config(delta_config2, backbone_model=model2)
        delta_model2.freeze_module(set_state_dict = False)
        delta_model2.log(delta_ratio=True, trainable_ratio=True, visualization=True)
    # make the flat pet into tensor again
    if "train_itp" in flat2:
        flat2 = flat2["train_itp"]
        tmp_dict2 = model2.state_dict()
        keys = []
        for k in tmp_dict2.keys():
            if "adapter" in k or "classifier" in k:
                keys.append(k)
        shapes = [tmp_dict2[k].size() for k in keys]
        lens = [tmp_dict2[k].numel() for k in keys]
        pos = 0
        for i in range(len(keys)):
            param = flat2[pos:pos+lens[i]].view(shapes[i])
            tmp_dict2[keys[i]] = param
            pos += lens[i]
        model2.load_state_dict(tmp_dict2)
    ...
    for x in np.linspace(0, 1, 26):
        ...
        test_metrics = {"eval_accuracy": 0, "eval_loss": 0}
        ...
        if data_args.task_name is None:
            ...
            for predict_dataset, task in zip(predict_datasets, tasks):
                ...
                test_metrics["eval_samples"] = min(max_predict_samples, len(predict_dataset))
        ...
        best_config, best_mm_performance, df = write_result(training_args.output_dir, result_name, metrics["eval_accuracy"], best_dev_performance, metrics["eval_loss"], test_metrics["eval_accuracy"], test_metrics["eval_loss"], df, "~", "acc", x)
