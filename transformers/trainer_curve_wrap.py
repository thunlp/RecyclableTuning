...
class WrapModelCF:
    def __init__(self, module: nn.Module, args, model1: nn.Module, model2: nn.Module, device="cpu"):
        super().__init__()
        self.args = args
        self.device = device
        ...
        fix_dict1 = [v for v in model1.state_dict().values()][1:]
        fix_dict2 = [v for v in module.state_dict().values()][1:]
        ...
        # Det the fixed point from scratch
        fix1 = torch.tensor([]).cuda()
        for v in fix_dict1:
            fix1 = torch.cat((fix1, v.flatten().cuda()), dim=0)
        fix2 = torch.Tensor([]).cuda()
        for v in fix_dict2:
            fix2 = torch.cat((fix2, v.flatten().cuda()),dim=0)
        self.fix_point = 0.5 * fix1 + 0.5 * fix2

        self.fix_point.requires_grad = False
