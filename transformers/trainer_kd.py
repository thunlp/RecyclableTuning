...
class Trainer:
    ...
    def __init__(
        ...
        if intrinsic != -1:
            ...
            model, wrap = itp_find_intrinsic(model, self.args, teacher_model, model2, intrinsic=intrinsic, device="cuda",)
        ...
        if self.place_model_on_device:
            ...
            self._move_model_to_device(teacher_model, args.device)
    ...
    def compute_loss(self, model, inputs, is_train=False, return_outputs=False, is_predict=False):
        """
        ...
        if self.beta != -1 and is_train:
            # Added to get hidden state!
            inputs["return_dict"] = True
            inputs["output_hidden_states"] = True

        if self.intrinsic != -1 and not is_predict:
            outputs = model(**inputs, x=1)
        else:
        ...
        if is_train:
            ...
            if self.beta != -1:
                ...
                total_loss = total_loss + self.beta * patience_loss
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    with self.autocast_smart_context_manager():
                        loss, outputs = self.compute_loss(model, inputs, return_outputs=True, is_predict=True)
