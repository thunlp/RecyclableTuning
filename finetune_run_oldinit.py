...
class ModelArguments:
    ...
    old_init_path: str = field(
        default=None, metadata={"help": "Path to the old delta model used as current initialization"}
    )
...
def main():
    ...
    import torch
    ori_tuned_model = torch.load(f"{model_args.old_init_path}/pytorch_model.bin")
    ...
    def eval():
        ...
        return float(metrics["eval_accuracy"]), float(metrics["eval_loss"])
    ...
    init_eval_result, init_eval_loss = eval()
    init_test_result, init_test_loss = predict()
    logger.info(f"============ Before Training eval result is {init_eval_result}, use it as zeroshot result! ============")
    logger.info(f"============ Before Training predict result is {init_test_result}, use it as zeroshot result! ============")
