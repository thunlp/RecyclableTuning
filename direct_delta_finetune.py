...
class ModelArguments:
    ...
    ori_model_path1: str = field(
        metadata={"help": "Path to the pretrained model1"}
    )
    ori_model_path2: str = field(
        metadata={"help": "Path to pretrained model model2"}
    )
...
def main():
    ...
    model_dict1 = {k: v for (k, v) in model1.state_dict().items()}
    model_dict2 = {k: v for (k, v) in model2.state_dict().items()}
    
    all_sub_keys = []
    for k in model_dict.keys():
        if "classifier" not in k and "position_ids" not in k:
            all_sub_keys.append(k)
    tmp_dict = {k: model_dict[k] - model_dict1[k] + model_dict2[k] for k in all_sub_keys}
    for k in model_dict.keys():
        if "classifier" in k:
            tmp_dict[k] = model_dict[k]
    model_dict2.update(tmp_dict)
